		...
		<div className="text-muted-foreground mt-6 fixed flex flex-col items-center justify-center w-screen h-screen gap-4">
			...
			<div className="flex h-72 opacity- 0 duration-300 w-5xl items-center justify-center gap-16 select-none">
				...
				<button
					...
					style={{
						pointerEvents: selected === "" ? "auto" : "none",
						filter: !selected || selected === "NTE" ? "brightness(1)" : "brightness(0.5)",
					}}
					className="flex duration-200 group hover:-mt-2 active:scale-90 p-6 rounded-md aspect-3/2 max-w-56.5 min-h-70 flex-col items-center wuwa-font"
				>
					<div
						className="w-full aspect-square bg-wuwa-accent/0 group-hover:bg-wuwa-accent duration-300 rounded-full blur-3xl -mb-[100%]"
						style={{ background: selected === "NTE" ? "var(--wuwa-accent)" : "" }}
					/>
					<img
						src="/NTELogo.png"
					...
					/>
					<label className="mt-6 text-xl leading-tight text-center">{GAME_NAMES.NTE}</label>
				</button>
			</div>
		</div>
	);
